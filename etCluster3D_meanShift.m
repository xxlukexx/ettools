function clus = etCluster3D_meanShift(gaze, bandwidth, timeScale, removeFirstSecs)
    ...
    if ~exist('removeFirstSecs', 'var') || isempty(removeFirstSecs)
        removeFirstSecs = 0;
    end
    ...
    % optionally remove first x ms of gaze, useful if a previous scene has
    % lingering effects
    if removeFirstSecs > 0
        s1 = 1;
        s2 = find(gaze.Time >= removeFirstSecs, 1, 'first');
        gaze.Missing(s1:s2, :) = true;
    end
